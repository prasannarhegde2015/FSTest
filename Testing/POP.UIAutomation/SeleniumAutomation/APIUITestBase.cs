...
namespace SeleniumAutomation
{
    public class APIUITestBase
    {
        ...
        public void scanPumpUpDynaCardfromCygNetDDS(string strwell)
        {
            WellDTO[] allwells = WellService.GetAllWells();
            WellDTO well = allwells.FirstOrDefault(x => x.Name == strwell);
            string wellid = well.Id.ToString();
            DynacardService.ScanAndAnalyzeDynacard(wellid, ((int)CardType.Full).ToString(), "runAnalysis");
        }
            ...
            try
            {
                ...
                if (_systemSettingsToRemove != null)
                {
                    foreach (var systemSetting in _systemSettingsToRemove)
                    {
                        ...
                        {
                            Trace.WriteLine(string.Format("Failed to remove system setting {0}: {1}", systemSetting.Id, ex.Message));
                        }
                    }
                }
        ...
        public AssetDTO AddAndGetAsset(AssetDTO assetToAdd)
        {
